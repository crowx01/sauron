...
# merge into any existing keys (env, permissions, model...) rather than overwriting the file
if [ -f "$TARGET" ]; then
  jq --argjson add "$JSON" '. * $add' "$TARGET" > "${TARGET}.tmp" && mv "${TARGET}.tmp" "$TARGET"
else
  echo "$JSON" | jq . > "$TARGET"
fi
ok "wrote $TARGET"
